...
- Presence shows a standing room of simulated people that follows the clock — emptiest (about 570) around 03:00 UTC, fullest (about 830) twelve hours later, wandering a few either side, and the same number for every visitor (`src/lib/presence/baseline.ts`, about 70% of them sharing an activity and a drink, reshuffled hourly but always ranked working > studying > reading > creating and coffee > tea > water), with real sessions counted on top. It is added once, in `usePresence`; adapters report only real sessions, and nothing else may pad or invent numbers.
