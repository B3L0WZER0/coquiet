...
4. `npm run contrast` (needs a dev server) — the text sits over the photograph, so a new room can fail AA on its own. Every element must pass. It audits only what has changed since it last passed: a new room costs about fifteen seconds, while touching a token or the veil re-audits all of them (three minutes, four at a time).
