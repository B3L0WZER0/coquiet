...
If presence sync is unavailable: `The quiet room is open.` If nobody else is present: `You're opening the room. Others will join.` The live number is the standing room (320–580 simulated people, following the hour of the day) plus real sessions.
...
Bottom left, one persistent line: `● [n] here now` — the standing room (320–580 simulated people, following the hour of the day) plus currently-entered sessions from the active adapter, never raw page views.
...
15. The count is always the 320–580 standing room plus real sessions, including when only one tab is open.
