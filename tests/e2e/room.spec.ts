...
test('a custom 1 / 10 session is honoured exactly as typed', async ({ page }) => {
  ...
  await expect(timer).toContainText('Timer1:00');
...
test('the custom field can be emptied with backspace', async ({ page }) => {
  ...
  await focus.blur();
  await expect(page.getByRole('button', { name: /Focus timer/ })).toContainText('Timer45:00');
});
...
test('an out-of-range custom value corrects itself visibly', async ({ page }) => {
  ...
  await expect(page.getByRole('button', { name: /Focus timer/ })).toContainText('Timer3:00:00');
});
...
test.describe('the timer does not outlive the page', () => {
  ...
  test('a running session is gone after a refresh', async ({ page }) => {
    ...
    await page.reload();
    await enter(page);
    await expect(page.getByRole('button', { name: /Focus timer/ })).toContainText('Timer25:00');
  });
  ...
  test('an idle custom length does not come back either', async ({ page }) => {
    ...
    await focus.fill('7');
    await focus.blur();
    await expect(page.getByRole('button', { name: /Focus timer/ })).toContainText('Timer7:00');
